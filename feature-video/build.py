"""Compose the 1920x1080 Google Play feature/promo video for Ascend (v2).
Clean vertical-slide transitions, balanced captions, proper Google Play badge.
"""
import math, numpy as np
from PIL import Image, ImageDraw, ImageFilter
import imageio_ffmpeg
from ascend import (font, icon, text, rrect, tw, logo_mark, _rgb,
                    INDIGO, INDIGO2, VIOLET, BG as APP_BG)
import screens
...
# ---------- brand background ----------
def make_bg():
    base = screens.vgrad((0,0,W,H), "#6d5cf0", "#191641", angle=145).convert("RGBA")
...
# ---------- phone shell ----------
SCALE = 1.07
IW, IH = round(screens.DW*SCALE), round(screens.DH*SCALE)
PADP = round(11*SCALE)
...
def make_shell():
    ...
    d.rounded_rectangle([2*SS,2*SS,(OW-2)*SS,(OH-2)*SS],radius=(ROUT-2)*SS,outline=(255,255,255,20),width=2*SS)
...
SHADOW=Image.new("RGBA",(OW+200,OH+200),(0,0,0,0))
ImageDraw.Draw(SHADOW).rounded_rectangle([100,120,OW+100,OH+92],radius=ROUT,fill=(8,6,26,160))
SHADOW=SHADOW.filter(ImageFilter.GaussianBlur(52))

SCR={n:getattr(screens,n)().resize((IW,IH),Image.LANCZOS)
     for n in ["home","search","tracker","optimizer","mock","navigator"]}
...
# ---------- timeline ----------
# eyebrow, [headline lines], subline, screen, accent
PHONE=[
 ...
 ("LIVE COPILOT",["Your live","interview copilot"],"Real-time answers during the call","navigator","#b3a6ff"),
]
INTRO,OUT,SCENE,TR = 3.6,4.2,3.7,0.5
T0=INTRO
TOTAL=INTRO+len(PHONE)*SCENE+OUT

CXT=132                      # caption left x
PX=W-OW//2-225               # phone center x
PY=H//2
...
def _pill(d,x,y,label,acc,a):
    ...
    fg=(22,18,42) if dark else (255,255,255)
    d.text((x+pad,y+h/2),label,font=f,fill=fg+(int(255*a),),anchor="lm")

def google_play_badge(scale=1.0):
    bw,bh=int(486*scale),int(132*scale)
    b=Image.new("RGBA",(bw,bh),(0,0,0,0)); d=ImageDraw.Draw(b)
    d.rounded_rectangle([0,0,bw-1,bh-1],radius=int(24*scale),fill=(0,0,0,255),
                        outline=(150,150,165,255),width=max(1,int(1.5*scale)))
    # play triangle (4-colour), supersampled
    s=4; th=int(58*scale); tw_=int(50*scale)
    tx,ty=int(40*scale),(bh-th)//2
    tri=Image.new("RGBA",(tw_*s,th*s),(0,0,0,0)); td=ImageDraw.Draw(tri)
    Wt,Ht=tw_*s,th*s; A=(0,0); Bp=(0,Ht); P=(Wt,Ht//2); C=(int(Wt*0.30),Ht//2)
    td.polygon([A,P,C],fill=(0,224,255,255))       # top  – blue/cyan
    td.polygon([Bp,P,C],fill=(255,61,84,255))      # bottom – red
    td.polygon([A,Bp,C],fill=(0,224,150,255))      # left – green
    mid=((C[0]+P[0])//2,Ht//2)                      # apex wedge – amber
    td.polygon([mid,(P[0],Ht//2-int(Ht*0.16)),(P[0],Ht//2+int(Ht*0.16))],fill=(255,200,60,255))
    tri=tri.resize((tw_,th),Image.LANCZOS)
    b.alpha_composite(tri,(tx,ty))
    txx=tx+tw_+int(26*scale)
    ef=font(int(20*scale),600); ex=txx
    for ch in "GET IT ON":            # manual letter-spacing
        d.text((ex,int(40*scale)),ch,font=ef,fill=(196,196,206),anchor="lm")
        ex+=tw(d,ch,ef)+int(3*scale)
    d.text((txx-2,int(86*scale)),"Google Play",font=font(int(40*scale),700),fill=(255,255,255),anchor="lm")
    return b
BADGE=google_play_badge(1.0)

def _paste_phone(img,screen_a,off=0,screen_b=None,dy=0):
    """phone at fixed position; off>0 slides screen_a up & screen_b in from below."""
    x=int(PX-OW//2); y=int(PY-OH//2+dy)
    img.paste(SHADOW,(x-100,y-100),SHADOW)
    img.paste(SHELL,(x,y),SHELL)
    inner=Image.new("RGB",(IW,IH),_rgb(APP_BG))
    inner.paste(screen_a,(0,-int(off)))
    if screen_b is not None:
        inner.paste(screen_b,(0,IH-int(off)))
    img.paste(inner,(x+PADP,y+PADP),INNER_MASK)

def _dots(d,x,y,idx):
    for i in range(len(PHONE)):
        on=i==idx; w=34 if on else 12
        d.rounded_rectangle([x,y,x+w,y+10],radius=5,
                            fill=(255,255,255,235) if on else (255,255,255,70)); x+=w+10
...
def frame(t):
    ...
    if t>TOTAL-OUT:
        ...
        if b>0.02:
            ...
            bb=BADGE.resize((max(1,bw),max(1,bh)),Image.LANCZOS)
            img.paste(bb,(W//2-bw//2,int(H//2+150-bh//2)),bb)
        return np.asarray(img)

    lt=t-T0; idx=min(len(PHONE)-1,int(lt//SCENE)); local=lt-idx*SCENE
    # top-left persistent brand
    ...
    # phone: entrance on first scene, vertical slide between scenes
    float_y=math.sin(lt*1.05)*9
    if idx==0:
        enter=out_cubic(clamp(lt/0.7))
        _paste_phone(img,SCR[PHONE[0][3]],0,None,(1-enter)*250+float_y)
    elif local<TR:
        e=smooth(local/TR)
        _paste_phone(img,SCR[PHONE[idx-1][3]],e*IH,SCR[PHONE[idx][3]],float_y)
    else:
        _paste_phone(img,SCR[PHONE[idx][3]],0,None,float_y)

    # caption: appears after the slide, fades out before next slide
    ...
    block_h=40+24+len(lines)*86+14+34
    top=int(H*0.52-block_h/2)
    if ca>0.02:
        _pill(d,CXT-dx,top,eb,acc,ca)
        yy=top+40+24+30
        for ln in lines:
            _ltext(d,CXT-dx,yy,ln,font(74,800),"#ffffff",ca); yy+=86
        yy+=2
        _ltext(d,CXT-dx,yy,sub,font(30,500),(224,222,246),ca)
